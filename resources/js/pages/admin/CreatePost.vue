<template>
	<div>
		...
		<div class="lg:flex justify-between items-center mb-6">
            <p class="text-2xl font-semibold mb-2 lg:mb-0">Create a new post</p>
        </div>
		...
		<div v-if="isWritePermitted">
			...
			<div class="input-field">
				<Button size="large" @click="save" :loading="isCreating" :disabled="isCreating"><Icon type="md-create" /> {{isCreating ? 'Please wait...' : 'Create post'}}</Button>
			</div>
		</div>    
    </div>
</template>
...
export default {
    ...
    async onSave(response){
		let data = response;
		// 'blocks' is an array in editor.js object containing all elemets of a the post
    ...
    async created(){
		if(this.getTags.length == 0 || this.getCategories.length == 0){
